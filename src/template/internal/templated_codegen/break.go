...
func (Break) Name() string { return "Break" }

func (template *Break) writeValue(
...
func (_template *Break) WriteTo(_output io.Writer) (int64, error) {
	_numWritten := int64(0)
...
`))
	...
	// break.template:14:6
	{
		_n, _err := _output.Write([]byte(`break
`))
		_numWritten += int64(_n)
		if _err != nil {
			return _numWritten, _err
