...
def main():
    max_length = max(len(i) for i in INSTRUCTIONS)
    template = '    %%-%ss = 0x%%x' % max_length

    result = ['// AUTO-GENERATED by gen_op_codes.py']
    result.append('object OpCode {')
    print template
    result.extend([template % (n, i) for i, n in enumerate(INSTRUCTIONS)])
    result.append('}')

    f = open('op_codes.scala', 'w')
    f.write('\n'.join(result))
    f.close()
